...
/// HTTP client that forwards IPO application submissions from
/// `ipo-applier` to the Playwright-driven `ipo-browser` service.
///
/// Only `apply_for_ipo` is meaningful for this adapter — the other
/// `BrokerBrowserPort` methods are intentionally disabled because
/// `ipo-applier` never owns the result-check or stock-scrape flows.
#[derive(Debug, Clone)]
...
impl BrokerBrowserPort for BrowserServiceClient {
    async fn fetch_ipo_stocks(&self) -> Result<Vec<ScrapedStock>, DomainError> {
        Err(DomainError::HttpClientError {
            reason: "fetch_ipo_stocks is not used by ipo-applier".to_string(),
        })
    }
    ...
    async fn test_connection(
        ...
        Err(DomainError::HttpClientError {
            reason: "test_connection is not used by ipo-applier".to_string(),
        })
    }
    ...
    async fn check_lottery_result(
        ...
        Err(DomainError::HttpClientError {
            reason: "check_lottery_result is not used by ipo-applier".to_string(),
        })
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn apply_for_ipo_translates_success_response() {
    ...
    #[tokio::test]
    async fn apply_for_ipo_decodes_failure_reason() {
        let server = MockServer::start().await;
        ...
        assert_eq!(
            ...
            ApplicationResult::Failure {
                reason: "browser timeout".to_string()
            }
        );
    }

    #[tokio::test]
    async fn apply_for_ipo_decodes_already_applied_variant() {
        let server = MockServer::start().await;
    ...
    #[tokio::test]
    async fn apply_for_ipo_decodes_insufficient_balance_variant() {
        let server = MockServer::start().await;
