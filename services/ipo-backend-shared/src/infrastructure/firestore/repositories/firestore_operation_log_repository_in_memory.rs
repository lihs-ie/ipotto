...
pub struct FirestoreOperationLogRepositoryInMemory {
    documents: Arc<Mutex<BTreeMap<String, OperationLogDocument>>>,
}

impl FirestoreOperationLogRepositoryInMemory {
    pub fn new() -> Self {
        Self::default()
    }
}
...
impl OperationLogRepository for FirestoreOperationLogRepositoryInMemory {
...
mod tests {
    ...
    use super::FirestoreOperationLogRepositoryInMemory;
    ...
    async fn filters_operation_logs() {
        let repository = FirestoreOperationLogRepositoryInMemory::new();
        let first = build_log(
