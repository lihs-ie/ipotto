...
pub struct FirestoreExclusionRepositoryInMemory {
    documents: Arc<Mutex<BTreeMap<String, ExclusionDocument>>>,
}

impl FirestoreExclusionRepositoryInMemory {
    pub fn new() -> Self {
        Self::default()
    }
}
...
impl ExclusionRepository for FirestoreExclusionRepositoryInMemory {
    async fn find_by_id(
        &self,
...
mod tests {
    ...
    use super::FirestoreExclusionRepositoryInMemory;
    use crate::domain::{
    ...
    async fn saves_exists_and_deletes_exclusion() {
        let repository = FirestoreExclusionRepositoryInMemory::new();
