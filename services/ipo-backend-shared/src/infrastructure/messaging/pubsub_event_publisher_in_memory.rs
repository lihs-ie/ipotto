...
#[derive(Debug, Clone)]
pub struct PubSubEventPublisherInMemory {
    service_name: String,
    published_messages: Arc<Mutex<Vec<String>>>,
}

impl PubSubEventPublisherInMemory {
...
#[async_trait]
impl EventPublisherPort for PubSubEventPublisherInMemory {
    async fn publish(
        &self,
        event_type: &str,
        aggregate_id: &str,
...
mod tests {
    ...
    use super::PubSubEventPublisherInMemory;
    use crate::{
    ...
    async fn publishes_serialized_envelope_with_expected_metadata() {
        let publisher = PubSubEventPublisherInMemory::new("ipo-service");
