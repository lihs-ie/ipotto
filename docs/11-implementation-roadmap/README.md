...
- **Phase 3 Sprint 6** (2FA 画像認証 ACL):
  - 6.1 IMAP / Gmail API クライアント 未着手 (PR #b 予定)。
  - 6.2 キーワード正規表現抽出 (120s / 3s polling) 未着手 (PR #b 予定)。
  ...
  - 6.4 3 段防御線 (session restore → mail OTP → manual fallback) 未着手 (PR #c 予定)。
  - 6.5 画像認証失敗時 SEV1 通知 未着手 (PR #c 予定)。
